...
def create_src_dest_dicts_from_ref(config_dir, mask_name,var_name, start_year, final_year, start_month, final_month, start_day, final_day):
    dest_files = []
    ...
    for dest_file in dest_files:
        dest_files_out.append('L1_BC_'+mask_name+'_'+var_name+'.'+dest_file+'.bin')
        source_files = []
        index_set = []
        for pair in size_dict[dest_file]:
            source_files.append(mask_name+'_'+var_name+'.'+pair[0]+'.bin')
            index_set.append(pair[1])
        source_file_read_dict['L1_BC_'+mask_name+'_'+var_name+'.'+dest_file+'.bin'] = source_files
        source_file_read_index_sets['L1_BC_'+mask_name+'_'+var_name+'.'+dest_file+'.bin'] = index_set
...
def read_boundary_variable_to_L2_points(L2_run_dir, mask_name, var_name,
    ...
    for s in range(len(source_files)):
        ...
        if var_name=='UVEL':
            var_file = os.path.join(L2_run_dir,'dv', 'L1_'+mask_name + '_BC_mask_UVEL.'+file_suffix)
            ...
            var_file = os.path.join(L2_run_dir,'dv', 'L1_'+mask_name +'_BC_mask_VVEL.'+file_suffix)
            var_grid = np.fromfile(var_file, dtype='>f4')
            timesteps_in_file = int(np.size(var_grid) / (Nr_in * N))
            var_grid = -1*np.reshape(var_grid, (timesteps_in_file, Nr_in, N))
        elif var_name=='UICE':
            var_file = os.path.join(L2_run_dir,'dv', 'L1_'+mask_name + '_BC_mask_UICE.'+file_suffix)
            var_grid = np.fromfile(var_file, dtype='>f4')
            ...
            # var_file = os.path.join(L2_run_dir, 'dv', mask_name + '_i' + str(layer), 'UVEL',
            #                         mask_name + '_i' + str(layer) + '_mask_UVEL.' + file_suffix)
            var_file = os.path.join(L2_run_dir,'dv', 'L1_'+mask_name +'_BC_mask_VICE.'+file_suffix)
            var_grid = np.fromfile(var_file, dtype='>f4')
            timesteps_in_file = int(np.size(var_grid) / (N))
            ...
            var_file = os.path.join(L2_run_dir,'dv', 'L1_'+mask_name +'_BC_mask_' + var_name +'.'+ file_suffix)
            var_grid = np.fromfile(var_file, dtype='>f4')
            if var_name in ['ETAN','UICE','VICE','HSNOW','HEFF','AREA']:
                timesteps_in_file = int(np.size(var_grid) / (N))
                var_grid = np.reshape(var_grid, (timesteps_in_file, 1, N))
...
def downscale_L2_bc_field_to_L1(df, mask_name, var_name,
                                L2_points, L2_values,
                                L2_wet_grid_3D_points, L2_wet_grid_on_L1_3D_points,
    ...
    if mask_name=='south' or mask_name=='north':
        if var_name=='ETAN':
            L1_boundary_var = np.zeros((nTimestepsOut, np.shape(L1_YC_subset)[0], np.shape(L1_YC_subset)[1]))
        else:
    ...
    # print('          -  L2_wet_grid_on_L1_subset: ' + str(np.shape(L2_wet_grid_on_L1_3D_points)))
...
def create_bc_field(config_dir, L1_model_name, mask_name,
    ...
    L1_XC, L1_YC, L1_wet_grid_3D = read_grid_geometry_from_nc(config_dir,L1_model_name,boundary_var_name)
    ...
    nc_dict_file = os.path.join(config_dir,'L2',L2_model_name,'namelist','L1_dv_mask_reference_dict.nc')
    ...
    for dest_file in dest_files:
        if dest_file not in []:#os.listdir(os.path.join(output_dir,mask_name,boundary_var_name)):
            ...
            L2_wet_grid_on_L1_3D_points = np.copy(L2_wet_grid_3D_points)

            print('    - Downscaling the output to the new boundary')
            L1_boundary_var = downscale_L2_bc_field_to_L1(df, mask_name, boundary_var_name,
                                                          L2_points, L2_values,
                                                          L2_wet_grid_3D_points, L2_wet_grid_on_L1_3D_points,
                                                          L1_XC_subset, L1_YC_subset, L1_wet_grid_3D_subset)
...
def create_bc_fields_via_interpolation(config_dir, L1_model_name, proc_id,
    ...
    mask_name_list = ['south','east','west',
                      'south','east','west',
                      'south','east','west',
                      'south','east','west',
                      'south','east','west',
    ...
    dest_files, source_file_read_dict, source_file_read_index_sets = \
        create_src_dest_dicts_from_ref(config_dir, mask_name, var_name, start_year, final_year, start_month, final_month, start_day, final_day)

    create_bc_field(config_dir, L1_model_name, mask_name,
