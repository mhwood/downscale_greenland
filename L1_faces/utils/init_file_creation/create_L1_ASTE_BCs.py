...
def read_grid_tile_geometry_to_boundaries(ordered_nonblank_tiles, compact_tile_size,
    ...
    if len(northern_tiles)>0:
        ...
        northern_wetgrid = np.zeros((90, len(northern_tiles) * compact_tile_size, ))
        counter = 0
    ...
    if len(southern_tiles)>0:
        ...
        southern_wetgrid = np.zeros((90, len(southern_tiles) * compact_tile_size,))
        counter = 0
    ...
    if len(eastern_tiles)>0:
        ...
        eastern_wetgrid = np.zeros((90, len(eastern_tiles) * compact_tile_size,))
        counter = 0
    ...
    if len(western_tiles)>0:
        ...
        western_wetgrid = np.zeros((90, len(western_tiles) * compact_tile_size,))
        counter = 0
...
def create_L1_BCs(config_dir,model_name,var_name,sNx,sNy,ordered_nonblank_tiles,tile_face_index_dict,
    ...
    for boundary in ['west']:#,'south','east','north']:
        ...
        if len(tile_numbers) > 0:
            ...
            if boundary in ['north','south']:
                output_grid = np.zeros((np.shape(aste_grid)[0],np.size(delR),sNx*len(tile_numbers)))
            else:
                output_grid = np.zeros((np.shape(aste_grid)[0], np.size(delR), sNy * len(tile_numbers)))
            ...
            if var_name in ['UVEL','VVEL']:
                output_file = os.path.join(config_dir,'L1',model_name,'input','obcs','L1_BC_'+boundary+'_'+var_name+'_rotated.bin')
            else:
                output_file = os.path.join(config_dir, 'L1', model_name, 'input', 'obcs',
                                           'L1_BC_' + boundary + '_' + var_name + '.bin')
