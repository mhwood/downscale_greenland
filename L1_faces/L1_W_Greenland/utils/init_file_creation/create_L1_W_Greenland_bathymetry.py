...
def fill_unconnected_areas(Lf,interpolated_bathy_faces, sNx, sNy):

    # stitched_bathy = np.concatenate([interpolated_bathy_faces[1],np.rot90(interpolated_bathy_faces[3],k=3)],axis=0)
    stitched_bathy = Lf.read_grid_faces_to_stitched_grid(interpolated_bathy_faces, dim=2)
    ...
    output_faces = Lf.read_stitched_grid_to_faces(stitched_bathy, sNx, sNy, dim=2)
    # print(np.shape(output_faces[3]))
    # print(np.shape(output_faces[5]))
...
def create_L1_W_Greenland_bathymetry(config_dir, model_name, ecco_dir, sNx, sNy, face_size_dict, llc, print_status):
    ...
    print(os.listdir(os.path.join(config_dir,'L1',model_name,'utils')))
    ...
    for face in faces:
        ...
        if face == 5:
            ...
            bathy_grid[300:310,480:510] = 0
            # close off hudson bay
            bathy_grid[150:160, 225:280] = 0
    ...
    interpolated_bathy_faces = fill_unconnected_areas(Lf,interpolated_bathy_faces, sNx, sNy)
