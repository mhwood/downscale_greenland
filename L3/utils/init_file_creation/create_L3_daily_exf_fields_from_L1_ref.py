...
def create_src_dest_dicts_from_ref(config_dir, L1_model_name, var_name, start_year, final_year, start_month, final_month, start_day, final_day):
    ...
    f = open(os.path.join(config_dir,'L1', L1_model_name, 'run','dv', 'L3_exf_dest_ref.txt'))
    dict_str = f.read()
    f.close()
    ...
    for dest_file in dest_files:
        dest_files_out.append('L3_exf_'+var_name+'.'+dest_file+suffix)
        source_files = []
        index_set = []
        for pair in size_dict[dest_file]:
            source_files.append('L3_surface_mask_'+var_name+'.'+pair[0]+'.bin')
            index_set.append(pair[1])
        source_file_read_dict['L3_exf_'+var_name+'.'+dest_file+suffix] = source_files
        source_file_read_index_sets['L3_exf_'+var_name+'.'+dest_file+suffix] = index_set
...
def read_exf_variable_to_L1_points(L1_run_dir, var_name,
    ...
    for s in range(len(source_files)):
        ...
        if var_name in ['UWIND','VWIND']:
            u_var_file = os.path.join(L1_run_dir, 'dv', 'L3_surface_mask_UWIND.' + file_suffix)
            ...
            v_var_file = os.path.join(L1_run_dir, 'dv', 'L3_surface_mask_VWIND.' + file_suffix)
            ...
            var_file = os.path.join(L1_run_dir, 'dv', 'L3_surface_mask_' + var_name + '.' + file_suffix)
...
def create_exf_field(config_dir, L1_model_name, L3_model_name,
    ...
    source_faces,source_rows,source_cols = read_mask_reference_from_nc_dict(nc_dict_file, 'L3_surface')
...
def create_exf_fields_via_interpolation(config_dir, L3_model_name, L1_model_name, proc_id,
                                        sNx, sNy, ordered_nonblank_tiles, ordered_nonblank_rotations,
                                        faces, ordered_tiles_faces_dict,
    ...
    create_exf_field(config_dir, L1_model_name, L3_model_name,
                    sNx, sNy, ordered_nonblank_tiles, ordered_nonblank_rotations,
                    faces, ordered_tiles_faces_dict,
                    var_name, dest_files,
