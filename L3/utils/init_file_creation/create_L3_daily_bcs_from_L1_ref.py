...
def create_src_dest_dicts_from_ref(config_dir, L1_model_name, mask_name,var_name, start_year, final_year, start_month, final_month, start_day, final_day):
    ...
    f = open(os.path.join(config_dir,'L1', L1_model_name, 'run','dv', 'L3_BC_dest_ref.txt'))
    dict_str = f.read()
    f.close()
...
def read_boundary_variable_to_L1_points(L1_run_dir, mask_name, var_name,
    ...
    for s in range(len(source_files)):
        ...
        if var_name in ['UVEL','VVEL']:
            u_var_file = os.path.join(L1_run_dir,'dv', 'L3_'+mask_name + '_BC_mask_UVEL.'+file_suffix)
            ...
            v_var_file = os.path.join(L1_run_dir, 'dv', 'L3_' + mask_name + '_BC_mask_VVEL.' + file_suffix)
            ...
            u_var_file = os.path.join(L1_run_dir, 'dv', 'L3_' + mask_name + '_BC_mask_UICE.' + file_suffix)
            u_var_grid = np.fromfile(u_var_file, dtype='>f4')
            timesteps_in_file = int(np.size(u_var_grid) / (N))
            u_var_grid = np.reshape(u_var_grid, (timesteps_in_file, 1, N))

            v_var_file = os.path.join(L1_run_dir, 'dv', 'L3_' + mask_name + '_BC_mask_VICE.' + file_suffix)
            v_var_grid = np.fromfile(v_var_file, dtype='>f4')
            ...
            var_file = os.path.join(L1_run_dir,'dv', 'L3_'+mask_name +'_BC_mask_' + var_name +'.'+ file_suffix)
