...
def get_dest_file_list(var_name, n_rows_L3, n_cols_L3, year):
    ...
    for month in range(1, 13):
        ...
        for day in range(1, nDays + 1):
            if var_name in ['UWIND','VWIND']:
                dest_file = 'L3_exf_' + var_name + '_' + str(year) + '{:02d}'.format(month)+ '{:02d}'.format(day) + '_rotated.bin'
            else:
                dest_file = 'L3_exf_' + var_name + '_' + str(year) + '{:02d}'.format(month) + '{:02d}'.format(day) + '.bin'
            dest_files.append(dest_file)
            nTimesteps = 4
            total_timesteps += nTimesteps
            dest_file_shapes[dest_file] = (nTimesteps, n_rows_L3, n_cols_L3)
...
def stack_daily_exf_files_to_one(config_dir, config_name, var_name, AngleCS, AngleSN, dest_files, dest_file_shapes,total_timesteps,print_level):
    ...
    for dest_file in dest_files:
        ...
        if var_name in ['UWIND','VWIND']:
            ...
            u_var_grid = np.fromfile(os.path.join(config_dir, 'L3', config_name, 'input', 'exf', 'UWIND', u_dest_file), '>f4')
            u_var_grid = np.reshape(u_var_grid, dest_file_shapes[dest_file])
            v_var_grid = np.fromfile(os.path.join(config_dir, 'L3', config_name, 'input', 'exf', 'VWIND', v_dest_file),'>f4')
...
def combine_and_rotate_L3_daily_exf_files(config_dir, config_name, proc_id,start_year,final_year,print_level):
    ...
    n_rows_L3 = np.shape(AngleCS)[0]
    n_cols_L3 = np.shape(AngleCS)[1]
    ds.close()
    ...
    for year in years:
        ...
        dest_files, dest_file_shapes, total_timesteps = get_dest_file_list(var_name, n_rows_L3, n_cols_L3, year)
        ...
        output_file = os.path.join(config_dir, 'L3',config_name, 'input', 'exf', 'L3_exf_' + var_name + '_'+str(year))
        if print_level >= 1:
