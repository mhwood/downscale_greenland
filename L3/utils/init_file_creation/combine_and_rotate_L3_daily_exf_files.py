...
def get_dest_file_list(var_name,n_rows_L3,n_cols_L3,
            ...
            for day in range(1, nDays + 1):
                ...
                if test_date >= start_date and test_date <= final_date:
                    if var_name in ['UWIND','VWIND']:
                        dest_file = 'L3_exf_' + var_name + '.' + str(year) + '{:02d}'.format(month)+ '{:02d}'.format(day) + '_rotated.bin'
                    else:
                        dest_file = 'L3_exf_' + var_name + '.' + str(year) + '{:02d}'.format(month) + '{:02d}'.format(
                            day) + '.bin'
...
def combine_and_rotate_L3_daily_exf_files(config_dir, config_name, proc_id,
    ...
    dest_files, dest_file_shapes, total_timesteps = get_dest_file_list(var_name, n_rows_L3, n_cols_L3,
                                                                       start_year, final_year, start_month, final_month, start_day, final_day)
