...
def create_L3_ASTE_seaice_pickup_file(config_dir, model_name, ordered_aste_tiles, ordered_aste_tile_rotations):
    ...
    aste_XC, aste_YC, aste_hfacC, aste_delR, aste_subset_bounds = read_aste_grid_geometry(aste_dir, ordered_aste_tiles,
    ...
    for vn in range(len(var_names)):
        ...
        if var_name not in []:  # used for testing
            ...
            # if var_name.lower() in ['sitices']:
            #     interp_field = np.zeros((7, np.shape(XC)[0], np.shape(XC)[1]))
            # else:
