...
def create_L3_Disko_Bay_files(config_dir):
    ...
    if np.any(np.array(steps)>2):
        ...
        if not os.path.exists(grid_path):
            sys.exit("Need to make the grid netcdf file for this model for reference\n"
                     "   - Run the stitch_L3_Disko_Bay_nc_grid_files_for_ref.py script with the -f 1 flag (and -r 150 -c 210)\n"
                     "   - Run the *for_grid model\n"
                     "   - Run the stitch_L3_Disko_Bay_nc_grid_files_for_ref.py script")

    # step 3: make the initial conditions
    if 3 in steps:
        print('Step 3: Creating the pickup (initial conditions) file for the ' + L3_model_name + ' model')
        import create_L3_Disko_Bay_pickup as cp
    ...
    if 4 in steps:
        ...
        import create_L3_Disko_Bay_seaice_pickup as cp
    ...
    if 5 in steps:
        ...
        import create_L3_Disko_Bay_exf as ce
    ...
    if 6 in steps:
        ...
        import create_L3_Disko_Bay_BCs as cbc
    ...
    if 7 in steps:
        ...
        import create_L3_Disko_Bay_iceplume as cip
...
if __name__ == '__main__':
    ...
    create_L3_Disko_Bay_files(config_dir)
