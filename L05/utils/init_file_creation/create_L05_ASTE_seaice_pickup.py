...
def rotate_interpolated_faces_to_domain(var_names,tile_number, tile_face_index_dict,
    ...
    siuice_grid_index = var_names.index('siUICE')
    sivice_grid_index = var_names.index('siVICE')
    natural_siuice = interp_grid_faces[siuice_grid_index][face][:,min_row:min_row+sNy,min_col:min_col+sNx]
    natural_sivice = interp_grid_faces[sivice_grid_index][face][:, min_row:min_row + sNy, min_col:min_col + sNx]
    siuice, sivice = rotate_velocity_vectors_to_domain(AngleCS, AngleSN,
                                                   natural_siuice, natural_sivice)
    interp_grid_faces[siuice_grid_index][face][:, min_row:min_row + sNy, min_col:min_col + sNx] = siuice
    interp_grid_faces[sivice_grid_index][face][:, min_row:min_row + sNy, min_col:min_col + sNx] = sivice
...
def create_L05_ASTE_seaice_pickup_file(config_dir,model_name,sNx,sNy,ordered_nonblank_tiles,tile_face_index_dict,
    ...
    print('    - Creating the seaice pickup file for the '+model_name+' model from ASTE data')
    ...
    pickup_file = 'pickup_seaice_it55warm.0000000007'
    var_names, var_grids, global_metadata = af.read_aste_seaice_pickup_to_stiched_grid(aste_dir, pickup_file, ordered_aste_tiles, ordered_aste_tile_rotations)

    # C = plt.imshow(var_grids[4][0,:,:], origin='lower',vmin=-1,vmax=1,cmap='seismic')
    # plt.colorbar(C)
    # plt.show()

    uice_grid = var_grids[var_names.index('siUICE')]
    vice_grid = var_grids[var_names.index('siVICE')]
    uice_grid_rotated, vice_grid_rotated = af.rotate_aste_grids_to_natural_grids(uice_grid, vice_grid, aste_AngleCS, aste_AngleSN)
    var_grids[var_names.index('siUICE')] = uice_grid_rotated
    var_grids[var_names.index('siVICE')] = vice_grid_rotated

    tice_grid = var_grids[var_names.index('siTICES')]
    tice_grid = tice_grid[:1,:,:]
    var_grids[var_names.index('siTICES')]=tice_grid
    var_names[var_names.index('siTICES')]='siTICE'
    ...
    for vn in range(len(output_var_names)):
        interp_grid = {}
        if output_var_names[vn].lower() == 'sitices':
            depth_levels = seaice_temp_Nr
        else:
            depth_levels = 1
        for face in faces:
        ...
        for tile_col in range(len(ordered_nonblank_tiles[0])):
            ...
            for vn in range(len(var_names)):
                ...
                if var_name not in []:  # used for testing
                    ...
                    domain_wet_cells_3D = read_tile_wet_cells_from_run_for_grid_dir(run_for_grid_dir, var_name,
                                                                                    tile_face, n_tiles, tile_number)
    ...
    #                 # plt.show()
                    ...
                    interp_field = df.downscale_3D_seaice_field(aste_XC, aste_YC,
    ...
    pickup_grid = stack_grids_to_compact_pickup(faces, interp_grid_faces, var_names, compact_tile_size)
    ...
    output_file = os.path.join(output_dir, 'pickup_seaice.' + '{:010d}'.format(1))
    dtype = '>f8'
