...
def read_llc_face_geometry(ecco_dir, face, llc=540):
...
def read_llc_bathymetry_file_to_faces(grid_file,llc=540):
...
def create_bathymetry(config_dir, model_name, ecco_dir):
    ...
    bathy_file = os.path.join(ecco_dir,'LLC'+str(llc)+'_Files','input_init','BATHY_2')# 'bathy_llc'+str(llc))
    bathy_faces = read_llc_bathymetry_file_to_faces(bathy_file,llc=540)
    ...
    output_file = os.path.join(config_dir, 'L05', model_name, 'input', 'bathymetry_unmasked.bin')
