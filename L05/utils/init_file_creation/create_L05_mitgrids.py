...
def create_mitgrids(config_dir, config_name, ecco_dir,
    ...
    for face in range(1,6):
        if face in face_dimensions.keys():
            ...
            output_file = os.path.join(config_dir,'L05',config_name,'input','tile' + '{:03d}'.format(face) + '.mitgrid')
    ...
    dict_file = os.path.join(config_dir, 'L05', config_name, 'namelist', 'face_dimensions.txt')
    f = open(dict_file,'w')
