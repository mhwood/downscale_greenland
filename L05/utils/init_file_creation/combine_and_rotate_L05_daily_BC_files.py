        ...
        for month in range(1, 13):
            ...
            for day in range(1, nDays + 1):
                ...
                if test_date >= start_date and test_date <= final_date:
                    dest_file = 'L05_BC_' + boundary + '_' + var_name + '.' + str(year) + '{:02d}'.format(
...
def stack_daily_bc_files_to_one(config_dir, model_name, boundary, var_name,
    ...
    for dest_file in dest_files:
        ...
        if var_name in ['UICE','VICE','UVEL','VVEL']:
            ...
            u_var_grid = np.fromfile(os.path.join(config_dir, 'L05',model_name, 'input', 'obcs', boundary, 'U'+var_name[1:], u_dest_file),'>f4')
            v_var_grid = np.fromfile(os.path.join(config_dir, 'L05',model_name, 'input', 'obcs', boundary, 'V'+var_name[1:], v_dest_file),'>f4')
            ...
            var_grid = np.fromfile(os.path.join(config_dir, 'L05', model_name, 'input', 'obcs', boundary, var_name, dest_file), '>f4')
...
def combine_L05_daily_BC_files(config_dir, model_name, Nr, sNx, sNy, ordered_nonblank_tiles,
    ...
    boundary_list = ['east', 'south', 'west',
                      ...
                      'east', 'south', 'west',
                      'east', 'south', 'west']
    boundary = boundary_list[proc_id % len(var_name_list)]
    ...
    output_file = os.path.join(config_dir, 'L05',model_name, 'input', 'obcs', 'L05_BC_'+boundary+'_' + var_name + '.bin')
