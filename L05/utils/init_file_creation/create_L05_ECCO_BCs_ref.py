...
def find_dv_files_to_read(config_dir, mask_name, var_name):#, start_iter, final_iter):
    ...
    for file_name in os.listdir(dv_dir):
        if mask_name in file_name and var_name in file_name:
    ...
    for i in range(len(file_iters_sorted)):
        ...
        iter_midpoints = tf.dv_file_name_iter_to_iter_midpoints(file_iter, iters_per_output, iters_per_file)
        # print(file_iter, iters_per_output, iters_per_file,iter_midpoints)
        iter_midpoint_dict[file_name] = iter_midpoints

    return(file_names, iter_midpoint_dict)


def create_destination_file_list(config_dir,mask_name, var_name,# start_iter, final_iter,
                                 file_names, iter_midpoint_dict):
    sys.path.insert(1, os.path.join(config_dir, 'utils','init_file_creation'))
    import time_functions as tf
    ...
    date_bounds = []
    for year in range(2002,2003):#1992, 2023):
    ...
    for date_set in date_bounds:
        ...
        dest_file = 'L05_BC_'+str(mask_name)+'_'+str(var_name)+'.'+str(date_set[0].year)+'{:02d}'.format(date_set[0].month)+'{:02d}'.format(date_set[0].day)+'.bin'
    ...
    for dest_file in dest_files:
        ...
        dest_file_iters = np.arange(dest_file_iter_0,dest_file_iter_1+1,iters_per_output)
        dest_file_iters += iters_per_output / 2
        # print('      - Iteration range: '+str(np.min(dest_file_iters))+' to '+str(np.max(dest_file_iters)))
...
def create_bc_fields_reference_dict(config_dir):

    print('Creating the bc field reference')
    ...
    file_names, iter_midpoint_dict = find_dv_files_to_read(config_dir, mask_name, var_name)#, start_iter, final_iter)
    ...
    output_file = os.path.join(config_dir,'L0','run','dv','daily_bc_source_ref.txt')
    ...
    dest_files, source_file_read_dict, source_file_read_index_sets = create_destination_file_list(config_dir,mask_name, var_name,# start_iter,final_iter,
                                                                                                  file_names,
                                                                                                  iter_midpoint_dict)
    ...
    output_file = os.path.join(config_dir,'L0','run','dv','daily_bc_dest_ref.txt')
    f = open(output_file,'w')
    f.write(output)
    f.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument("-s", "--start_iter", action="store",
                        help="The start iteration to store w.r.t L0.", dest="start_iter",
                        type=int, required=False, default=175680)

    parser.add_argument("-f", "--final_iter", action="store",
                        help="The end iteration to store w.r.t L0.", dest="final_iter",
                        type=int, required=False, default=876960)
    ...
    create_bc_fields_reference_dict(config_dir, start_iter, final_iter)