...
def read_L2_CE_Greenland_boundary_condition(config_dir, L2_model_name, boundary, field_name,
                                            Nr, n_rows_L2, n_cols_L2,  n_timesteps):

    if boundary=='south' or boundary=='north':
        boundary_file = os.path.join(config_dir,'L2',L2_model_name,'input','obcs',
                                     'L2_BC_'+boundary+'_'+field_name.upper()+'.bin')
        boundary_grid = np.fromfile(boundary_file,'>f4')
        boundary_grid = boundary_grid.reshape((n_timesteps, Nr, n_cols_L2))

    if boundary == 'east' or boundary == 'west':
        boundary_file = os.path.join(config_dir, 'L2', L2_model_name, 'input', 'obcs',
                                     'L2_BC_' + boundary + '_' + field_name.upper() + '.bin')
        boundary_grid = np.fromfile(boundary_file, '>f4')
        boundary_grid = boundary_grid.reshape((n_timesteps, Nr, n_rows_L2))


    return(boundary_grid)

def create_2D_BC_plot(config_dir, L2_model_name, var_name, timestep,
    ...
    output_file = os.path.join(config_dir,'L2',L2_model_name,'plots','init_files',L2_model_name+'_BC_'+var_name+'_'+str(timestep)+'.png')
    plt.savefig(output_file)
    plt.close(fig)


def create_3D_BC_plot(config_dir, L2_model_name, var_name, timestep,
                   north_boundary, south_boundary, east_boundary):
...
def plot_L2_CE_Greenland_BCs(config_dir, L2_model_name, print_level):
    ...
    n_rows_L2 = np.shape(XC)[0]
    n_cols_L2 = np.shape(XC)[1]
    ds.close()
    ...
    for var_name in var_names:
        ...
        north_boundary = read_L2_CE_Greenland_boundary_condition(config_dir, L2_model_name, 'north', var_name,
                                                                 Nr, n_rows_L2, n_cols_L2, n_timesteps)
        north_boundary = north_boundary[timestep, :, :]

        south_boundary = read_L2_CE_Greenland_boundary_condition(config_dir, L2_model_name, 'south', var_name,
                                                                 Nr, n_rows_L2, n_cols_L2, n_timesteps)
        south_boundary = south_boundary[timestep, :, :]

        east_boundary = read_L2_CE_Greenland_boundary_condition(config_dir, L2_model_name, 'east', var_name,
                                                                 Nr, n_rows_L2, n_cols_L2, n_timesteps)
        east_boundary = east_boundary[timestep, :, :]

        if var_name in ['THETA', 'SALT', 'UVEL', 'VVEL']:
            create_3D_BC_plot(config_dir, L2_model_name, var_name, timestep,
                           north_boundary, south_boundary, east_boundary)
        else:
            create_2D_BC_plot(config_dir, L2_model_name, var_name, timestep,
                              north_boundary, south_boundary, east_boundary)
