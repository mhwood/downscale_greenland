...
def plot_L2_NEGIS_init_fields(config_dir):

    L2_model_name = 'L2_NEGIS'
    pickup_iteration = 1052640
    ...
    if 1 in steps:
        ...
        import plot_L2_NEGIS_mitgrid_components as pmc
        pmc.plot_L2_NEGIS_mitgrid_components(config_dir)

    # step 2: plot the bathymetry and wetgrid
    if 2 in steps:
        print('Step 2: Plotting the bathymetry for the ' + L2_model_name + ' model')
        import plot_L2_NEGIS_bathymetry as pb
        pb.plot_L2_NEGIS_bathymetry(config_dir)

    # step 3: plot the initial conditions
    if 3 in steps:
        print('Step 3: Plotting the pickup (initial conditions) fields for the ' + L2_model_name + ' model')
        import plot_L2_NEGIS_pickup_fields as cp
        cp.plot_L2_NEGIS_pickup(config_dir, pickup_iteration)
    ...
    if 4 in steps:
        ...
        import plot_L2_NEGIS_seaice_pickup_fields as csp
        csp.plot_L2_NEGIS_seaice_pickup(config_dir, pickup_iteration)
    ...
    if 5 in steps:
        ...
        import plot_L2_NEGIS_exf_fields as cef
        cef.plot_L2_NEGIS_exfs(config_dir)

    # step 6: plot the boundary conditions
    if 6 in steps:
        print('Step 6: Plotting the boundary conditions for the ' + L2_model_name + ' model')
        import plot_L2_NEGIS_BC_fields as pbc
        pbc.plot_L2_NEGIS_BCs(config_dir, L2_model_name, print_level)
...
if __name__ == '__main__':
    ...
    plot_L2_NEGIS_init_fields(config_dir)
