...
def read_CTD_points_from_shp(config_dir, model_name):
    shp_file = os.path.join(config_dir,'L2',model_name,'input','dv_shp','CTD_NEG')
...
def create_dv_masks(config_dir, L2_model_name, print_level):
    ...
    for boundary in ['iceplume','shelfice','CTD_NEG','Zach_Fjd','79N_Fjd']:#,'DJG_Fjord','Sund_Entrance'] #CTD
        if print_level >= 1:
            print('    - Creating the ' + boundary +' mask')

        if boundary=='shelfice':
            shelfice_grid = np.fromfile(os.path.join(config_dir,'L2',L2_model_name,
                                                     'input','shelfice','ice_thickness_1992'), '>f4')
            shelfice_grid = np.reshape(shelfice_grid, np.shape(L2_XC))
            mask_indices = []
            mask_grid = np.zeros_like(L2_XC)
            counter = 1
            for i in range(np.shape(L2_XC)[0]):
                for j in range(np.shape(L2_XC)[1]):
                    if shelfice_grid[i,j]!=0:
                        mask_indices.append([i,j])
                        mask_grid[i,j] = counter
                        counter += 1
            mask_indices = np.array(mask_indices)
        elif boundary == 'iceplume':
            iceplume_grid = np.fromfile(os.path.join(config_dir,'L2',L2_model_name,
                                                     'input','iceplume','L2_iceplume_mask.bin'), '>f4')
            iceplume_grid = np.reshape(iceplume_grid, np.shape(L2_XC))
            mask_indices = []
            mask_grid = np.zeros_like(L2_XC)
            counter = 1
            for i in range(np.shape(L2_XC)[0]):
                for j in range(np.shape(L2_XC)[1]):
                    if iceplume_grid[i,j]!=0:
                        mask_indices.append([i,j])
                        mask_grid[i,j] = counter
                        counter += 1
            mask_indices = np.array(mask_indices)
