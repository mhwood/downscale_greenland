...
def plot_L3_Disko_Bay_init_fields(config_dir):
    ...
    if 1 in steps:
        ...
        import plot_L3_Disko_Bay_mitgrid_components as pmc
        pmc.plot_L3_Disko_Bay_mitgrid_components(config_dir)

    # step 2: plot the bathymetry and wetgrid
    if 2 in steps:
        print('Step 2: Plotting the bathymetry for the ' + L3_model_name + ' model')
        import plot_L3_Disko_Bay_bathymetry as pb
        pb.plot_L3_Disko_Bay_bathymetry(config_dir)

    # step 3: plot the initial conditions
    if 3 in steps:
        print('Step 3: Plotting the pickup (initial conditions) fields for the ' + L3_model_name + ' model')
        import plot_L3_Disko_Bay_pickup_fields as cp
        cp.plot_L3_Disko_Bay_pickup(config_dir, pickup_iteration)
    ...
    if 4 in steps:
        ...
        import plot_L3_Disko_Bay_seaice_pickup_fields as csp
        csp.plot_L3_Disko_Bay_seaice_pickup(config_dir, pickup_iteration)
    ...
    if 5 in steps:
        ...
        import plot_L3_Disko_Bay_exf_fields as cef
        cef.plot_L3_Disko_Bay_exfs(config_dir)

    # step 6: plot the boundary conditions
    if 6 in steps:
        print('Step 6: Plotting the boundary conditions for the ' + L3_model_name + ' model')
        import plot_L3_Disko_Bay_BC_fields as pbc
        pbc.plot_L3_Disko_Bay_BCs(config_dir, L3_model_name, print_level)
