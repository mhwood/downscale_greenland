...
def split_mitgrid_file(config_dir,rows,cols,for_grid):
    ...
    L1_size = size_dict['L3_Nuup_Kangerlua']
    n_rows = L1_size[0]
    n_cols = L1_size[1]
    ...
    mitgrid_file = os.path.join(config_dir,'mitgrids','L3_Nuup_Kangerlua.mitgrid')
    ...
    for ri in range(nr):
        for ci in range(nc):
            tile_subset = entire_grid[:,rows*ri:rows*(ri+1)+1,
                                        cols*ci:cols*(ci+1)+1]
            output_file = os.path.join(config_dir,'L3','L3_Nuup_Kangerlua','input'+for_grid_suffix,'tile'+'{:03d}'.format(counter)+'.mitgrid')
