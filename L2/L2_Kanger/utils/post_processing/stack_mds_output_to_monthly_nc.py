...
def stack_data_to_nc(config_dir, run_dir, subset):
    ...
    for subset in subsets:
        ...
        subset_folder = os.path.join(config_dir, 'L2', L2_model_name, run_dir, 'diags', subset)
        iter_numbers = []
        ...
        if results_dir not in os.listdir(os.path.join(config_dir,'L2',L2_model_name)):
            os.mkdir(os.path.join(config_dir,'L2',L2_model_name,results_dir))
        if subset not in os.listdir(os.path.join(config_dir, 'L2',L2_model_name,results_dir)):
            os.mkdir(os.path.join(config_dir, 'L2', L2_model_name, results_dir, subset))
        output_dir = os.path.join(config_dir, 'L2', L2_model_name, results_dir, subset)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-r", "--run_dir", action="store",
                        help="The name of the run directory.", dest="run_dir",
                        type=str, required=True)
    ...
    stack_data_to_nc(config_dir, run_dir, subset)
