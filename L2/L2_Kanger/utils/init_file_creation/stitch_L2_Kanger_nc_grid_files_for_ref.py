...
def stitch_grid_files(config_dir):
    ...
    for row in range(8):
        row_numbers = []
        for col in range(6):
            row_numbers.append(counter)
            counter+=1
        ordered_tiles.append(row_numbers)
    Nr = 90
    sNx = 100
    sNy = 85
    ...
    if zero_rows>1:
        print('    - The grid has '+str(zero_rows)+' zero rows \n      - chopping off '+str(zero_rows-1)+'some of them to reduce computational time')
        new_rows = len(DRF)-zero_rows+1
        DRF=DRF[:new_rows]
        hFacC = hFacC[:new_rows,:,:]
        hFacS = hFacS[:new_rows, :, :]
        hFacW = hFacW[:new_rows, :, :]
        print('        - new shape: '+str(np.shape(hFacC)))
