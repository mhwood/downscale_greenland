...
def read_mask_reference_from_nc_dict(nc_dict_file,model_name,mask_name):
    ...
    elif model_name == 'L2_Santa_Barbara':
        grp = ds.groups['L2_SB_' + read_boundary]
    elif model_name == 'L2_NEGIS':
        grp = ds.groups['L2_NEG_' + read_boundary]
    else:
        grp = ds.groups[model_name+'_'+read_boundary]
...
def read_L1_boundary_variable_points(L1_run_dir, L2_model_name, boundary, var_name,
    ...
    for s in range(len(source_files)):
        ...
        elif L2_model_name == 'L2_Santa_Barbara':
            if var_name in ['UVEL','VVEL']:
                ...
                var_grid = np.fromfile(var_file, dtype='>f4')
        elif L2_model_name == 'L2_NEGIS':
            if var_name in ['UVEL','VVEL']:
                u_var_file = os.path.join(L1_run_dir, 'dv','L2_NEGIS',boundary, 'L2_NEG_' + boundary + '_mask_UVEL.' + file_suffix)
                u_var_grid = np.fromfile(u_var_file, dtype='>f4')
                v_var_file = os.path.join(L1_run_dir, 'dv', 'L2_NEGIS',boundary, 'L2_NEG_' + boundary + '_mask_VVEL.' + file_suffix)
                v_var_grid = np.fromfile(v_var_file, dtype='>f4')
            elif var_name in ['UICE','VICE']:
                u_var_file = os.path.join(L1_run_dir, 'dv', 'L2_NEGIS',boundary, 'L2_NEG_' + boundary + '_mask_UICE.' + file_suffix)
                u_var_grid = np.fromfile(u_var_file, dtype='>f4')
                v_var_file = os.path.join(L1_run_dir, 'dv', 'L2_NEGIS',boundary, 'L2_NEG_' + boundary + '_mask_VICE.' + file_suffix)
                v_var_grid = np.fromfile(v_var_file, dtype='>f4')
            else:
                var_file = os.path.join(L1_run_dir, 'dv', 'L2_NEGIS',boundary, 'L2_NEG_'+ boundary + '_mask_' +var_name +'.' + file_suffix)
                var_grid = np.fromfile(var_file, dtype='>f4')
        else:
            if var_name in ['UVEL','VVEL']:
                u_var_file = os.path.join(L1_run_dir, 'dv', L2_model_name,boundary, L2_model_name+'_' + boundary + '_mask_' + 'UVEL.' + file_suffix)
                u_var_grid = np.fromfile(u_var_file, dtype='>f4')
                v_var_file = os.path.join(L1_run_dir, 'dv', L2_model_name,boundary, L2_model_name+'_' + boundary + '_mask_' +'VVEL.' + file_suffix)
                v_var_grid = np.fromfile(v_var_file, dtype='>f4')
            elif var_name in ['UICE','VICE']:
                u_var_file = os.path.join(L1_run_dir, 'dv', L2_model_name,boundary, L2_model_name+'_' + boundary + '_mask_' +'UICE.' + file_suffix)
                u_var_grid = np.fromfile(u_var_file, dtype='>f4')
                v_var_file = os.path.join(L1_run_dir, 'dv', L2_model_name,boundary, L2_model_name+'_' + boundary + '_mask_' +'VICE.' + file_suffix)
                v_var_grid = np.fromfile(v_var_file, dtype='>f4')
            else:
                var_file = os.path.join(L1_run_dir, 'dv', L2_model_name,boundary, L2_model_name+'_'+ boundary + '_mask_' +var_name +'.' + file_suffix)
                var_grid = np.fromfile(var_file, dtype='>f4')
...
def create_bc_field(config_dir, L1_model_name, L2_model_name, mask_name,
    ...
    if L2_model_name=='L2_Santa_Barbara':
        nc_dict_file = os.path.join(config_dir,'L1',L1_model_name, 'input',L2_model_name+'_dv_mask_reference_dict_SB.nc')
    else:
        nc_dict_file = os.path.join(config_dir, 'L1', L1_model_name, 'input', L2_model_name+'_dv_mask_reference_dict.nc')
...
def create_bc_fields_via_interpolation(config_dir, L1_model_name, L2_model_name, boundaries, proc_id,
    ...
    if L2_model_name in ['L2_Disko_Bay','L2_Nares','L2_NEGIS']:
