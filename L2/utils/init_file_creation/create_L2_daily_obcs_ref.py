...
def find_dv_files_to_read(config_dir, model_name, Nr, averaging_period, seconds_per_iter,
                          mask_name, var_name, start_iter, final_iter):
    ...
    for file_name in os.listdir(dv_dir):
        if mask_name in file_name and var_name in file_name:
            ...
            iters_per_file = int(np.size(np.fromfile(os.path.join(dv_dir,file_name),'>f4'))/(N*Nr))
            print(iters_per_file)
    ...
    for i in range(len(file_iters_sorted)):
        ...
        iter_midpoints = tf.dv_file_name_iter_to_iter_midpoints(file_iter, iters_per_output, iters_per_file)
        if file_name == 'L2_north_BC_mask_THETA.0000004321.bin':
            iter_midpoints+=1
        # print(file_name)
...
def create_destination_file_list(config_dir, mask_name, averaging_period, seconds_per_iter,
    ...
    for date_set in date_bounds:
        ...
        if not iter_0>final_iter+1 and not iter_1<start_iter-1:
            dest_file = 'L2_BC_'+str(mask_name)+'_'+str(var_name)+'.'+str(date_set[0].year)+'{:02d}'.format(date_set[0].month)+'{:02d}'.format(date_set[0].day)+'.bin'
...
def create_bc_fields_reference_dict(config_dir, L05_model_name, Nr, averaging_period, seconds_per_iter):
    ...
    iter_count_check = 24#int(averaging_period/seconds_per_iter)

    print('Creating the bc field reference to cover iterations ' + str(start_iter) + ' to ' + str(final_iter))

    # calculate the file name iterations to read from
    #     along with the iterations these files cover
    file_names, iter_midpoint_dict = find_dv_files_to_read(config_dir, L05_model_name, Nr, averaging_period, seconds_per_iter,
                                                           mask_name, var_name, start_iter, final_iter)
    ...
    output_file = os.path.join(config_dir,'L05',L05_model_name,'run','dv','daily_bc_source_ref.txt')
    ...
    dest_files, source_file_read_dict, source_file_read_index_sets = create_destination_file_list(config_dir, mask_name,
                                                                                                  averaging_period,
                                                                                                  seconds_per_iter,
                                                                                                  var_name,
    ...
    output_file = os.path.join(config_dir,'L05',L05_model_name,'run','dv','daily_bc_dest_ref.txt')
    f = open(output_file,'w')
    f.write(output)
    f.close()
