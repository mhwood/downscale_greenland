...
def get_dest_file_list(mask_name, var_name, Nr, n_rows_L2,n_cols_L2, year):
    ...
    for month in range(1, 13):
        ...
        if var_name in ['UVEL','VVEL','UICE','VICE']:
            dest_file = 'L2_BC_'+mask_name+'_'+ var_name + '.' + str(year) + '{:02d}'.format(month) + '_rotated.bin'
        else:
            dest_file = 'L2_BC_'+mask_name+'_' + var_name + '.' + str(year) + '{:02d}'.format(month) + '.bin'
...
def stack_monthly_bc_files_to_one(config_dir, L2_model_name, mask_name, var_name, AngleCS, AngleSN, dest_files, dest_file_shapes,total_timesteps,print_level,read_from_unbalanced):
    ...
    for dest_file in dest_files:
        print('       - Adding fields to '+dest_file)

        if '.199201' in dest_file:
            # skipping first month because its not needed
            # spin up for L2 starts after this month (actually at 1993/1/1 as of this note)
            ...
            if var_name in ['UVEL','VVEL','UICE','VICE']:
                if 'VEL' in var_name:
                    ...
                    if read_from_unbalanced:
                        u_var_grid = np.fromfile(os.path.join(config_dir, 'L2', L2_model_name, 'input', 'obcs','unbalanced',mask_name, 'UVEL', u_dest_file), '>f4')
                        v_var_grid = np.fromfile(os.path.join(config_dir, 'L2', L2_model_name, 'input', 'obcs','unbalanced',mask_name, 'VVEL', v_dest_file),'>f4')
                    else:
                        u_var_grid = np.fromfile(os.path.join(config_dir, 'L2', L2_model_name, 'input', 'obcs', mask_name, 'UVEL',u_dest_file), '>f4')
                        v_var_grid = np.fromfile(os.path.join(config_dir, 'L2', L2_model_name, 'input', 'obcs', mask_name, 'VVEL',v_dest_file), '>f4')
                ...
                if 'ICE' in var_name:
                    ...
                    if read_from_unbalanced:
                        u_var_grid = np.fromfile(os.path.join(config_dir, 'L2', L2_model_name, 'input', 'obcs','unbalanced',mask_name, 'UICE', u_dest_file), '>f4')
                        v_var_grid = np.fromfile(os.path.join(config_dir, 'L2', L2_model_name, 'input', 'obcs','unbalanced',mask_name, 'VICE', v_dest_file),'>f4')
                    else:
                        u_var_grid = np.fromfile(os.path.join(config_dir, 'L2', L2_model_name, 'input', 'obcs', mask_name, 'UICE',u_dest_file), '>f4')
                        v_var_grid = np.fromfile(os.path.join(config_dir, 'L2', L2_model_name, 'input', 'obcs', mask_name, 'VICE',v_dest_file), '>f4')
                    u_var_grid = np.reshape(u_var_grid, dest_file_shapes[dest_file])
                    v_var_grid = np.reshape(v_var_grid, dest_file_shapes[dest_file])
                ...
                if read_from_unbalanced:
                    var_grid = np.fromfile(os.path.join(config_dir,'L2',L2_model_name, 'input','obcs','unbalanced',mask_name,var_name,dest_file),'>f4')
                else:
                    var_grid = np.fromfile(os.path.join(config_dir, 'L2', L2_model_name, 'input', 'obcs', mask_name, var_name,dest_file), '>f4')
...
def combine_and_rotate_L2_monthly_bcs(config_dir, L2_model_name, boundaries, var_names,
            ...
            for year in years:
                ...
                if read_from_unbalanced:
                    if var_name=='UVEL' and mask_name=='east':
                        output_file = os.path.join(config_dir, 'L2', L2_model_name, 'input', 'obcs', 'unbalanced',mask_name,var_name,
                                                   'L2_BC_'+mask_name+'_' + var_name + '_' + str(year)+'_unbalanced')
                        ...
                        if var_name not in os.listdir(os.path.join(config_dir, 'L2', L2_model_name, 'input', 'obcs', 'balanced',mask_name)):
                            os.mkdir(os.path.join(config_dir, 'L2', L2_model_name, 'input', 'obcs', 'balanced',mask_name,var_name))
                        output_file = os.path.join(config_dir, 'L2', L2_model_name, 'input', 'obcs', 'balanced',mask_name,var_name,
                                                   'L2_BC_' + mask_name + '_' + var_name + '_' + str(year))
                else:
                    if var_name=='UVEL' and mask_name=='east':
                        output_file = os.path.join(config_dir, 'L2', L2_model_name, 'input', 'obcs', 'annual',
                                                   'L2_BC_'+mask_name+'_' + var_name + '_' + str(year)+'_unbalanced')
                    elif var_name=='UVEL' and mask_name=='west':
                        output_file = os.path.join(config_dir, 'L2', L2_model_name, 'input', 'obcs', 'annual',
                                                   'L2_BC_'+mask_name+'_' + var_name + '_' + str(year)+'_unbalanced')
                ...
                if not os.path.exists(output_file):
                    ...
                    output_grid = stack_monthly_bc_files_to_one(config_dir, L2_model_name, mask_name, var_name,
...
if __name__ == '__main__':
    ...
    combine_and_rotate_L2_monthly_bcs(config_dir, L2_model_name, proc_id,
