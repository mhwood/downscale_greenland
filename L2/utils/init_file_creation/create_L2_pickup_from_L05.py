...
def rotate_directional_fields_to_natural(var_grids,var_names,L05_AngleCS, L05_AngleSN):
    uvel_grid = var_grids[var_names.index('Uvel')]
    vvel_grid = var_grids[var_names.index('Vvel')]
    natural_uvel_grid, natural_vvel_grid = rotate_grids_to_natural_grids(uvel_grid, vvel_grid, L05_AngleCS, L05_AngleSN)
    ...
    natural_gunm1_grid, natural_gvnm1_grid = rotate_grids_to_natural_grids(gunm1_grid, gvnm1_grid, L05_AngleCS,
                                                                           L05_AngleSN)
    ...
    natural_gunm2_grid, natural_gvnm2_grid = rotate_grids_to_natural_grids(gunm2_grid, gvnm2_grid, L05_AngleCS,
                                                                           L05_AngleSN)
...
def create_pickup_from_L05(config_dir, L05_model_name, L05_iteration, L2_model_name,
                           sNx, sNy, ordered_nonblank_tiles, ordered_nonblank_rotations,
                           faces, ordered_tiles_faces_dict):

    print('    - Creating the pickup file for the ' + L2_model_name + ' model from the '+L05_model_name+' model')
    ...
    L05_XC, L05_YC, L05_AngleCS, L05_AngleSN, L05_Hfac = read_L05_grid_tile_geometry(config_dir,L05_model_name,sNx, sNy, Nr_in,
                                                                        ordered_nonblank_tiles, ordered_nonblank_rotations,
                                                                        faces, ordered_tiles_faces_dict)


    # plt.subplot(2,3,1)
    # C = plt.imshow(L05_XC, origin='lower')
    # plt.colorbar(C)
    # plt.subplot(2, 3, 2)
    # C = plt.imshow(L05_YC, origin='lower')
    # plt.colorbar(C)
    # plt.subplot(2, 3, 3)
    # C = plt.imshow(L05_Hfac[0,:,:], origin='lower')
    # plt.colorbar(C)
    # plt.subplot(2, 3, 4)
    # C = plt.imshow(L05_AngleCS, origin='lower')
    # plt.colorbar(C)
    # plt.subplot(2, 3, 5)
    # C = plt.imshow(L05_AngleSN, origin='lower')
    # plt.colorbar(C)
    # plt.show()

    L2_XC, L2_YC, L2_AngleCS, L2_AngleSN = read_grid_geometry_from_nc(config_dir,L2_model_name)

    pickup_file = 'pickup.'+'{:010d}'.format(L05_iteration)
    pickup_file_path = os.path.join(config_dir,'L05',L05_model_name,'run',pickup_file)
    var_names, var_grids, pickup_metadata = read_pickup_to_stiched_grid(pickup_file_path, Nr_in, sNx, sNy,
                                                                        ordered_nonblank_tiles, ordered_nonblank_rotations,
                                                                        faces, ordered_tiles_faces_dict)

    var_grids, var_names = rotate_directional_fields_to_natural(var_grids, var_names, L05_AngleCS, L05_AngleSN)
    ...
    for vn in range(len(var_names)):
        ...
        if var_name not in []:  # used for testing
            ...
            L05_wet_cells = np.copy(L05_Hfac)
            L05_wet_cells[L05_wet_cells > 0] = 1

            if var_name.lower() not in ['etan', 'detahdt', 'etah']:
                var_grid, L05_wet_cells = df.interpolate_var_grid_faces_to_new_depth_levels(
                    var_grid, L05_wet_cells, delR_in, delR_out)
            ...
            L05_wet_cells_on_domain_3D = np.copy(domain_wet_cells_3D)

            interp_field = df.downscale_3D_field(L05_XC, L05_YC,
                                                 var_grid, L05_wet_cells,
                                                 L05_wet_cells_on_domain_3D,
            ...
            # plt.subplot(2, 3, 1)
            # plt.imshow(L05_wet_cells[0, :, :], origin='lower')
            # plt.subplot(2, 3, 2)
            # plt.imshow(L05_wet_cells_on_domain_3D[0, :, :], origin='lower')
            # plt.subplot(2, 3, 3)
    ...
    output_dir = os.path.join(config_dir, 'L2', L2_model_name, 'input')
    output_file = os.path.join(output_dir, 'pickup.' + '{:010d}'.format(L05_iteration*4))
    dtype = '>f8'
