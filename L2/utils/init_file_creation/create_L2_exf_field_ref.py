...
def create_L2_exf_ref_file(config_dir, L2_model_name, parent_model, print_level):

    if print_level>1:
        print('    - Creating the exf field reference for the '+L2_model_name)
    ...
    output_file = os.path.join(config_dir,'L1',parent_model,'run','dv','exf_source_ref.txt')
    ...
    output_file = os.path.join(config_dir,'L1',parent_model,'run','dv','exf_dest_ref.txt')
    f = open(output_file,'w')
    f.write(output)
    f.close()
