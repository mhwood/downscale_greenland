...
def create_shelfice_files(config_dir, model_name, print_level=1):
    ...
    phi0surf = create_surface_load_file(ice_topo, hFacC, Nr, drF, tRef, sRef, rhoConst=1030)
    ...
    ice_topo = np.array(ice_topo)
    phi0surf = np.array(phi0surf)
    ...
    plt.subplot(2, 2, 2)
    C = plt.imshow(ice_topo, origin='lower')
    plt.colorbar(C)
    ...
    input_dir = os.path.join(config_dir,'L2',model_name,'input')
    if 'shelfice' not in os.listdir(input_dir):
        os.mkdir(os.path.join(input_dir,'shelfice'))

    output_file = os.path.join(input_dir,'shelfice','ice_thickness.bin')
    ice_topo.ravel('C').astype('>f4').tofile(output_file)

    output_file = os.path.join(input_dir, 'shelfice', 'ice_phi0surf.bin')
