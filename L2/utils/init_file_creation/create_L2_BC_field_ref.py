...
def find_dv_files_to_read(config_dir, L1_model_name, L2_model_name, boundary, var_name, averaging_period, seconds_per_iter, points_per_output, Nr):
    ...
    file_iters = []
    n_files_in_files = []
    dv_dir = os.path.join(config_dir, 'L1', L1_model_name, 'run', 'dv', L2_model_name, boundary)
...
def create_destination_file_list(config_dir, var_name, file_names, iter_midpoint_dict, averaging_period, seconds_per_iter, print_level):
    ...
    for date_set in date_bounds:
        ...
        dest_file = 'L2_BC_'+str(var_name)+'.'+str(date_set[0].year)+'{:02d}'.format(date_set[0].month)+'.bin'
    ...
    for dest_file in dest_files:
        ...
        if len(source_file_names)>0:
            ...
            for file_name in source_file_read_dict[dest_file]:
                ...
                if print_level >= 5:
                    print('            - This file has iters '+str(np.min(source_file_iters))+' to '+str(np.max(source_file_iters))+' (n='+str(len(source_file_iters))+')')
                # try:
...
def create_L2_BC_ref_file(config_dir, L1_model_name, L2_model_name, print_level):
    ...
    if L2_model_name=='L2_NEGIS':
        model_name_short = 'L2_NEG'
    else:
        model_name_short = L2_model_name
    mask_ref_file = os.path.join(config_dir,'L1',L1_model_name, 'input', L2_model_name+'_dv_mask_reference_dict.nc')
    ds = nc4.Dataset(mask_ref_file)
    points_per_output = len(ds.groups[model_name_short+'_'+boundary].variables['source_rows'][:])
    ds.close()

    # calculate the file name iterations to read from
    #     along with the iterations these files cover
    file_names, iter_midpoint_dict = find_dv_files_to_read(config_dir, L1_model_name, L2_model_name, boundary, var_name, averaging_period, seconds_per_iter, points_per_output, Nr)
    ...
    output_file = os.path.join(config_dir,'L2',L2_model_name,'input',L2_model_name+'_BC_source_ref.txt')
    ...
    output_file = os.path.join(config_dir,'L2',L2_model_name,'input',L2_model_name+'_BC_dest_ref.txt')
    f = open(output_file,'w')
    f.write(output)
    f.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument("-m", "--L2_model_name", action="store",
                        help="The directory where the L1, L2, and L1 configurations are stored.", dest="L2_model_name",
                        type=str, required=True)
    ...
    create_L2_BC_ref_file(config_dir, L2_model_name, print_level=4)