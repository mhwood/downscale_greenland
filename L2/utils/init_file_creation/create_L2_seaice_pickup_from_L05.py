...
def create_seaice_pickup_from_L05(config_dir, L05_model_name, L05_iteration, L2_model_name,
    ...
    L05_XC, L05_YC, L05_AngleCS, L05_AngleSN, L05_Hfac = read_L05_grid_tile_geometry(config_dir,L05_model_name,sNx, sNy,
                                                                        ordered_nonblank_tiles, ordered_nonblank_rotations,
                                                                        faces, ordered_tiles_faces_dict)
    ...
    pickup_file = 'pickup_seaice.'+'{:010d}'.format(L05_iteration)
    ...
    for vn in range(len(var_names)):
        ...
        if var_name not in []:  # used for testing
            ...
            print('    L05_wet_cells shape: ',np.shape(L05_wet_cells))
            print('    domain_wet_cells shape: ', np.shape(domain_wet_cells))

            L05_wet_cells_on_domain = np.copy(domain_wet_cells)


            interp_field = df.downscale_3D_seaice_field(L05_XC, L05_YC,
                                                 var_grid, L05_wet_cells,
                                                 L05_wet_cells_on_domain,
            ...
            # plt.subplot(2, 3, 2)
            # plt.imshow(L05_wet_cells_on_domain[0, :, :], origin='lower')
            # plt.subplot(2, 3, 3)
    ...
    output_dir = os.path.join(config_dir, 'L2', L2_model_name, 'input')
    output_file = os.path.join(output_dir, 'pickup_seaice.' + '{:010d}'.format(L05_iteration*4))
    dtype = '>f8'
