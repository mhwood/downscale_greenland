...
def create_L3_pickup_file(config_dir, L1_model_name, L1_iteration, L3_model_name, zero_etan, print_level):

    if print_level>=1:
        print('    - Creating the pickup file for the ' + L3_model_name + ' model from the '+L1_model_name+' model')
    ...
    L3_XC, L3_YC, L3_AngleCS, L3_AngleSN, delR_out = read_grid_geometry_from_nc(config_dir,L3_model_name)
    Nr_out = len(delR_out)

    pickup_file = 'pickup.'+'{:010d}'.format(L1_iteration)
    pickup_file_path = os.path.join(config_dir,'L1_grid',L1_model_name,'run',pickup_file)
    ...
    for vn in range(len(var_names)):
        ...
        if var_name not in []:  # used for testing
            ...
            domain_wet_cells_3D = read_grid_mask_from_nc(config_dir,L3_model_name,var_name)
            ...
            if print_level >= 3:
                ...
                print('            - L3_XC shape: ' + str(np.shape(L3_XC)))
                print('            - L3_YC shape: ' + str(np.shape(L3_YC)))
            ...
            interp_field = df.downscale_3D_field(L1_XC, L1_YC,
                                                 ...
                                                 L3_XC, L3_YC, domain_wet_cells_3D,
            ...
            if var_name.lower() not in ['etan', 'detahdt', 'etah']:
                interp_field = np.zeros((len(delR_out), np.shape(L3_XC)[0], np.shape(L3_XC)[1]))
            else:
    ...
    interp_grids = rotate_interpolated_grids_to_domain(var_names, interp_grids, L3_AngleCS, L3_AngleSN)
