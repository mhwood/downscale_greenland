...
def read_L1_SE_Greenland_boundary_condition(config_dir, L1_model_name, boundary, field_name, sNx, sNy, Nr, n_timesteps):
    ...
    if boundary=='south':
        ...
        boundary_grid_1 = boundary_grid_1.reshape((n_timesteps, Nr, 3 * sNx))
        boundary_grid_1 = boundary_grid_1[:, :, -sNx:]

        boundary_file = os.path.join(config_dir,'L1',L1_model_name,'input','obcs',
                                     'L1_BC_'+boundary+'_'+field_name.upper()+'.bin')
        boundary_grid_2 = np.fromfile(boundary_file,'>f4')
        boundary_grid_2 = boundary_grid_2.reshape((n_timesteps, Nr, sNx*4))
        boundary_grid_2 = boundary_grid_2[:,:,:sNx*2]
    ...
    if boundary == 'east':
        boundary_file = os.path.join(config_dir, 'L1', L1_model_name, 'input', 'obcs',
                                     'L1_BC_east_' + field_name.upper() + '.bin')
        boundary_grid = np.fromfile(boundary_file, '>f4')
        boundary_grid = boundary_grid.reshape((n_timesteps, Nr, 3*sNx))
        boundary_grid = boundary_grid[:, :, :2*sNx]
    ...
    if boundary == 'north':
        ...
        boundary_grid_1 = np.fromfile(boundary_file, '>f4')
        boundary_grid_1 = boundary_grid_1.reshape((n_timesteps, Nr, 3 * sNx))
        boundary_grid_1 = 0*boundary_grid_1[:, :, -sNx:]
...
def plot_L1_SE_Greenland_BCs(config_dir, L1_model_name, sNx, sNy):
    ...
    for var_name in var_names:
        ...
        north_boundary = read_L1_SE_Greenland_boundary_condition(config_dir, L1_model_name, 'north', var_name,
                                                                 sNx, sNy, Nr, n_timesteps)
        north_boundary = north_boundary[timestep, :, :]

        print('    - Reading the south boundary')
        south_boundary = read_L1_SE_Greenland_boundary_condition(config_dir, L1_model_name, 'south', var_name,
                                                                 sNx, sNy, Nr, n_timesteps)
        south_boundary = south_boundary[timestep, :, :]

        print('    - Reading the west boundary')
        west_boundary = read_L1_SE_Greenland_boundary_condition(config_dir, L1_model_name, 'west', var_name,
                                                                 sNx, sNy, Nr, n_timesteps)
        west_boundary = west_boundary[timestep, :, :]

        print('    - Reading the east boundary')
        east_boundary = read_L1_SE_Greenland_boundary_condition(config_dir, L1_model_name, 'east', var_name,
                                                                 sNx, sNy, Nr, n_timesteps)
