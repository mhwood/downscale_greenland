...
def create_L1_N_Greenland_files(config_dir, ecco_dir):
    ...
    if 1 in steps:
        ...
        import create_L1_N_Greenland_mitgrid as cm
        cm.create_L1_N_Greenland_mitgrid_file(config_dir, L1_model_name, ecco_dir, print_level)

    # step 2: make the bathymetry
    if 2 in steps:
        print('Step 2: Creating the bathymetry file for the ' + L1_model_name + ' model')
        import create_L1_N_Greenland_bathymetry as cb
        cb.create_L1_N_Greenland_bathymetry(config_dir, ecco_dir, print_level)
    ...
    if np.any(np.array(steps)>2):
        ...
        if not os.path.exists(grid_path):
            sys.exit("Need to make the grid netcdf file for this model for reference\n"
                     "   - Run the stitch_L1_N_Greenland_nc_grid_files_for_ref.py script with the -f 1 flag (and -r 150 -c 210)\n"
                     "   - Run the *for_grid model\n"
                     "   - Run the stitch_L1_N_Greenland_nc_grid_files_for_ref.py script")

    # step 3: make the initial conditions
    if 3 in steps:
        print('Step 3: Creating the pickup (initial conditions) file for the ' + L1_model_name + ' model')
        import create_L1_N_Greenland_pickup as cp
    ...
    if 4 in steps:
        ...
        import create_L1_N_Greenland_seaice_pickup as cp
    ...
    if 5 in steps:
        ...
        import create_L1_N_Greenland_exf as ce
    ...
    if 6 in steps:
        ...
        import create_L1_N_Greenland_BCs as cbc
        cbc.create_BCs(config_dir, ecco_dir, print_level)

    # step 7: make the dv masks
    if 7 in steps:
        print('Step 7: Creating the diagnostic_vec masks for the ' + L1_model_name + ' model')
        import create_L1_N_Greenland_dv_masks as cdv
...
if __name__ == '__main__':
    ...
    create_L1_N_Greenland_files(config_dir, ecco_dir)
