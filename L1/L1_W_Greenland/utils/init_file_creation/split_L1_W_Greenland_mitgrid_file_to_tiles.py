...
def split_mitgrid_file(config_dir,rows,cols,for_grid):
    ...
    mitgrid_file = os.path.join(config_dir,'L1_grid','L1_W_Greenland','input','L1_W_Greenland.mitgrid')
    ...
    for ri in range(nr):
        for ci in range(nc):
            tile_subset = entire_grid[:,rows*ri:rows*(ri+1)+1,
                                        cols*ci:cols*(ci+1)+1]
            output_file = os.path.join(config_dir,'L1_grid','L1_W_Greenland','input'+for_grid_suffix,'tiles_'+str(n_tiles),
