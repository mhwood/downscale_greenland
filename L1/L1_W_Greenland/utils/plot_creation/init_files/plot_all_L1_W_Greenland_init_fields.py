...
def plot_L1_W_Greenland_init_fields(config_dir):
    ...
    faces = [3,5]
    face_size_dict = {3: (1 * sNy, 3 * sNx), 5: (3 * sNy, 3 * sNx)}
    ...
    if 1 in steps:
        ...
        import plot_L1_W_Greenland_mitgrid_components as pmc
        pmc.plot_L1_W_Greenland_mitgrid_components(config_dir, L1_model_name, faces, face_size_dict)

    # step 2: plot the bathymetry and wetgrid
    if 2 in steps:
        print('Step 2: Plotting the bathymetry for the ' + L1_model_name + ' model')
        import plot_L1_W_Greenland_bathymetry as pb
        pb.plot_L1_W_Greenland_bathymetry(config_dir, L1_model_name, sNx, sNy, faces, face_size_dict)

    # step 3: plot the initial conditions
    if 3 in steps:
        print('Step 3: Plotting the pickup (initial conditions) fields for the ' + L1_model_name + ' model')
        import plot_L1_W_Greenland_pickup_fields as cp
        cp.plot_L1_W_Greenland_pickup(config_dir, L1_model_name, pickup_iteration,
    ...
    if 4 in steps:
        ...
        import plot_L1_W_Greenland_seaice_pickup_fields as csp
        csp.plot_L1_W_Greenland_seaice_pickup(config_dir, L1_model_name, pickup_iteration,
                                               sNx, sNy, faces, face_size_dict)
    ...
    if 5 in steps:
        ...
        import plot_L1_W_Greenland_exf_fields as cef
        cef.plot_L1_W_Greenland_exfs(config_dir, L1_model_name, exf_shape,
                                      sNx, sNy, faces, face_size_dict)
    ...
    if 6 in steps:
        ...
        import plot_L1_W_Greenland_BC_fields as pbc
        pbc.plot_L1_W_Greenland_BCs(config_dir, L1_model_name, sNx, sNy)
