...
def read_L1_grid_tile_geometry(config_dir, model_name, Nr, sNx, sNy, ordered_nonblank_tiles, ordered_tile_rotations):
    ...
    return(stitched_XC, stitched_YC, stitched_XG, stitched_YG, stitched_AngleCS, stitched_AngleSN, stitched_Depth, DRF)
            #stitched_DXC, stitched_DYC,
            #stitched_hFacC, stitched_hFacS, stitched_hFacW,


def write_grid_to_nc(config_dir, model_name,
                     XC, YC, XG, YG, AngleCS, AngleSN, DRF, Depth):
...
def stitch_grid_files(config_dir):
    ...
    ordered_tile_rotations = [[0,0,0],[3,3,3]]
    Nr = 50
    sNx = 180
    sNy = 180

    XC, YC, XG, YG, AngleCS, AngleSN, Depth, DRF = read_L1_grid_tile_geometry(config_dir, model_name, Nr, sNx, sNy, ordered_tiles, ordered_tile_rotations)

    write_grid_to_nc(config_dir, model_name,
                     XC, YC, XG, YG, AngleCS, AngleSN,
                     DRF, Depth)
