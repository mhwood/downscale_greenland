...
def get_dest_file_list(model_name, boundary, var_name, Nr, sNx, sNy, tile_numbers, year):
    ...
    for boundary in ['south','east','west','north']:
        if boundary in os.listdir(os.path.join(config_dir,'L1',model_name,'input','obcs')):
            ...
            for year in range(start_year,final_year+1):
                ...
                dest_files, dest_file_shapes, total_timesteps = get_dest_file_list(model_name, boundary, var_name, Nr, sNx, sNy, tile_numbers, year)
                ...
                output_file = os.path.join(config_dir, 'L1', model_name, 'input', 'obcs', 'L1_BC_'+boundary+'_' + var_name+'_' + str(year))
                if print_level >= 1:
