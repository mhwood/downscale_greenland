...
def create_L1_BC_ref_file(config_dir, L1_model_name, averaging_period, seconds_per_iter, print_level, read_darwin = False):
    ...
    iters_per_file = int(86400/averaging_period)
    ...
    file_names, iter_midpoint_dict = find_dv_files_to_read(config_dir, L1_model_name, var_name,
                                                           averaging_period, seconds_per_iter, points_per_output, read_darwin)
    if print_level>=2:
    ...
    if read_darwin:
        output_file = os.path.join(config_dir,'L0','run_darwin','dv', model_prefix, L1_model_name+'_BC_source_ref.txt')
    else:
    ...
    for file_name in dest_files:
        ...
        if int(iter_count / iters_per_file) == 1:
            output += add_line[:-2]
        else:
            output += ''
        if print_level >= 2:
            print('        - Total iterations for this file: '+str(iter_count)+' (= '+str(iter_count/iters_per_file)+' days)')
        output+='],\n'
    output += '}'

    if read_darwin:
        output_file = os.path.join(config_dir,'L0','run_darwin','dv',model_prefix, L1_model_name+'_BC_dest_ref.txt')
    else:
