...
def plot_L1_CE_Greenland_init_fields(config_dir):
    ...
    pickup_iteration = 288

    L2_model_name = 'L2_CE_Greenland'

    if 'init_files' not in os.listdir(os.path.join(config_dir,'L1_grid',L1_model_name,'plots')):
        os.mkdir(os.path.join(config_dir,'L1_grid',L1_model_name,'plots','init_files'))

    sys.path.insert(1, os.path.join(config_dir, 'L1_grid', L1_model_name, 'utils', 'plot_creation','init_files'))
    ...
    if 1 in steps:
        ...
        pmc.plot_L1_CE_Greenland_mitgrid_components(config_dir, L1_model_name, n_rows, n_cols)
    ...
    #
    # # step 6: plot timeseries of the mean external forcing fields
    # if 6 in steps:
    #     print('Step 6: Plotting a timeseries of the external forcing conditions for the ' + L1_model_name + ' model')
    #     import plot_L1_CE_Greenland_exf_timeseries as cef
    #     cef.plot_L1_CE_Greenland_exf_timeseries(config_dir, L1_model_name, exf_shape,
    #                                             sNx, sNy, faces, face_size_dict)
    #
    # step 7: plot the boundary conditions at a random time step
    if 7 in steps:
        print('Step 7: Plotting the boundary conditions for the ' + L1_model_name + ' model')
        import plot_L1_CE_Greenland_BC_fields as pbc
        pbc.plot_L1_CE_Greenland_BCs(config_dir, L1_model_name, print_level)

    # step 8: plotting a timeseries of the boundary conditions for each boundary
    if 8 in steps:
        print('Step 8: Plotting a Hovmoller diagram of the boundary conditions timeseries for the ' + L1_model_name + ' model')
        import plot_L1_CE_Greenland_BC_timeseries as pbt
        pbt.plot_L1_CE_Greenland_BC_timeseries(config_dir, L1_model_name, print_level)
