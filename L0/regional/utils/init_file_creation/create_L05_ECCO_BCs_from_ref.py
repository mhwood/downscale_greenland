...
def create_src_dest_dicts_from_ref(config_dir, boundary, var_name,
    ...
    f = open(os.path.join(config_dir,'L0', 'run','dv', 'daily_bc_dest_ref.txt'))
    dict_str = f.read()
    f.close()
    ...
    for dest_file in dest_files:
        dest_files_out.append('L05_BC_'+boundary+'_'+var_name+'.'+dest_file+'.bin')
        source_files = []
        index_set = []
        for pair in size_dict[dest_file]:
            source_files.append(boundary+'_'+var_name+'.'+pair[0]+'.bin')
            index_set.append(pair[1])
        source_file_read_dict['L05_BC_'+boundary+'_'+var_name+'.'+dest_file+'.bin'] = source_files
        source_file_read_index_sets['L05_BC_'+boundary+'_'+var_name+'.'+dest_file+'.bin'] = index_set
...
def create_L05_BCs(config_dir,model_name,var_name,Nr,sNx,sNy,ordered_nonblank_tiles,tile_face_index_dict,
    ...
    for boundary in ['south','west','east']:
        ...
        if len(tile_numbers) > 0:
            ...
            if boundary not in os.listdir(os.path.join(config_dir,'L05',model_name,'input','obcs')):
                os.mkdir(os.path.join(config_dir,'L05',model_name,'input','obcs',boundary))
            if var_name not in os.listdir(os.path.join(config_dir, 'L05', model_name, 'input', 'obcs', boundary)):
                os.mkdir(os.path.join(config_dir, 'L05', model_name, 'input', 'obcs', boundary, var_name))
            ...
            for dest_file in dest_files:
                ...
                for tn in range(len(tile_numbers)):
                    ...
                    if tile_number>0:
                        ...
                        if var_name in ['AREA', 'HEFF', 'HSNOW']:
                            remove_zeros = False
                        else:
                            remove_zeros = True
                        ...
                        for timestep in range(np.shape(L0_boundary_values)[0]):
                            ...
                            interp_field = df.downscale_3D_points(L0_boundary_points,
                ...
                if var_name in ['UVEL','VVEL','UICE','VICE']:
                    output_file = os.path.join(config_dir, 'L05', model_name, 'input', 'obcs', boundary, var_name,dest_file[:-4]+'_rotated.bin')
                else:
                    output_file = os.path.join(config_dir, 'L05', model_name, 'input', 'obcs', boundary, var_name,dest_file)
