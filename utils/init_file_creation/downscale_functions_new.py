...
def spread_var_horizontally_in_wet_grid(var_grid,wet_grid,spread_mask):
...
def downscale_3D_field(L0_XC, L0_YC, L0_var, L0_wet_grid, L0_wet_grid_on_L1,
                       ...
                       mean_vertical_difference=0,fill_downward=True,printing=False,remove_zeros=False,L3_flag=False):
    ...
    for k in range(np.shape(L0_var)[0]):
        ...
        if continue_to_interpolation:
            ...
            if np.any(L1_wet_grid[k, :, :] > 0):
                ...
                spread_mask[L0_wet_grid_on_L1[k, :, :] == 0] = 0

                # mask out any values which should be 0'd based on the new bathy
                grid[L1_wet_grid[k, :, :] == 0] = 0
                spread_mask[L1_wet_grid[k, :, :] == 0] = 1

                # spread the the variable outward to new wet cells
                grid, n_remaining = spread_var_horizontally_in_wet_grid(grid, L1_wet_grid[k, :, :], spread_mask)
...
def downscale_3D_boundary_field(L0_XC, L0_YC, L0_var, L0_wet_grid, L0_wet_grid_on_L1,
                                boundary_points, L1_wet_grid,
                                mean_vertical_difference=0,fill_downward=True,
                                printing=False,remove_zeros=True):

    # full_grid = np.zeros_like(L1_wet_grid).astype(float)
    full_grid = np.zeros((np.shape(L0_var)[0],np.shape(boundary_points)[0]))
    ...
    for k in range(np.shape(L0_var)[0]):
        ...
        if continue_to_interpolation:
            ...
            if np.any(L1_wet_grid[k, :] > 0):
                ...
                if len(L0_points)>4:
                    grid = griddata(L0_points, L0_values, (boundary_points[:,0], boundary_points[:,1]), method='linear',fill_value=0)
                    grid = grid[:, 0]
                else:
