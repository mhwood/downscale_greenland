...
def read_aste_field_to_stiched_grid(aste_dir,field_name,ordered_aste_tiles,ordered_aste_tile_rotations,
                                    aste_AngleCS = [], aste_AngleSN = [], rotate_velocity = False):

    aste_Nr = 50
    aste_sNx = 90
    aste_sNy = 90

    full_field_started = False
    ...
    for r in range(len(ordered_aste_tiles)):
        ...
        for c in range(len(ordered_aste_tiles[r])):
            ...
            if field_name in ['UVEL', 'VVEL'] and rotate_velocity:
                file_path = os.path.join(aste_dir,'UVEL','UVEL'+'.'+'{:04d}'.format(aste_tile_row[c])+'.nc')
                ds = nc4.Dataset(file_path)
                uvel_grid = np.array(ds.variables['UVEL'][:,:,:,:])
                ds.close()

                file_path = os.path.join(aste_dir, 'VVEL',
                                         'VVEL' + '.' + '{:04d}'.format(aste_tile_row[c]) + '.nc')
                ds = nc4.Dataset(file_path)
                vvel_grid = np.array(ds.variables['VVEL'][:, :, :, :])
                ds.close()
            elif field_name in ['SIuice','SIvice'] and rotate_velocity:
                file_path = os.path.join(aste_dir, 'SIuice',
                                         'SIuice.' + '{:04d}'.format(aste_tile_row[c]) + '.nc')
                ds = nc4.Dataset(file_path)
                uice_grid = np.array(ds.variables['SIuice'][:, :, :])
                ds.close()
                uice_grid = uice_grid.reshape((np.shape(uice_grid)[0], 1, np.shape(uice_grid)[1], np.shape(uice_grid)[2]))

                file_path = os.path.join(aste_dir, 'SIvice',
                                         'SIvice.' + '{:04d}'.format(aste_tile_row[c]) + '.nc')
                ds = nc4.Dataset(file_path)
                vice_grid = np.array(ds.variables['SIvice'][:, :, :])
                ds.close()
                vice_grid = vice_grid.reshape((np.shape(vice_grid)[0], 1, np.shape(vice_grid)[1], np.shape(vice_grid)[2]))
            elif field_name in ['SIarea','SIheff','SIhsnow','ETAN']:
                file_path = os.path.join(aste_dir, field_name,
                                         field_name + '.' + '{:04d}'.format(aste_tile_row[c]) + '.nc')
                ds = nc4.Dataset(file_path)
                var_grid = np.array(ds.variables[field_name][:, :, :])
                ds.close()
                var_grid = var_grid.reshape((np.shape(var_grid)[0], 1, np.shape(var_grid)[1], np.shape(var_grid)[2]))
            else:
                file_path = os.path.join(aste_dir, field_name,
                                         field_name + '.' + '{:04d}'.format(aste_tile_row[c]) + '.nc')
                ds = nc4.Dataset(file_path)
            ...
            if not full_field_started:
                full_field_started = True
                if field_name in ['UVEL', 'VVEL'] and rotate_velocity:
                    uvel_aste_grid = np.zeros((np.shape(uvel_grid)[0],np.shape(uvel_grid)[1],
                                          len(ordered_aste_tiles)*aste_sNy,len(ordered_aste_tiles[0])*aste_sNx))
                    vvel_aste_grid = np.zeros((np.shape(vvel_grid)[0], np.shape(vvel_grid)[1],
                                               len(ordered_aste_tiles) * aste_sNy,
                                               len(ordered_aste_tiles[0]) * aste_sNx))
                elif field_name in ['SIuice','SIvice'] and rotate_velocity:
                    uice_aste_grid = np.zeros((np.shape(uice_grid)[0],np.shape(uice_grid)[1],
                                          len(ordered_aste_tiles)*aste_sNy,len(ordered_aste_tiles[0])*aste_sNx))
                    vice_aste_grid = np.zeros((np.shape(vice_grid)[0], np.shape(vice_grid)[1],
                                               len(ordered_aste_tiles) * aste_sNy,
                                               len(ordered_aste_tiles[0]) * aste_sNx))
                else:
                    aste_grid = np.zeros((np.shape(var_grid)[0], np.shape(var_grid)[1],
                                          len(ordered_aste_tiles) * aste_sNy, len(ordered_aste_tiles[0]) * aste_sNx))

            # rotate things as necessary
            for n in range(aste_rotation_row[c]):
                if field_name in ['UVEL', 'VVEL'] and rotate_velocity:
                    uvel_grid = np.rot90(uvel_grid, axes=(2, 3))
                    vvel_grid = np.rot90(vvel_grid, axes=(2, 3))
                elif field_name in ['SIuice', 'SIvice'] and rotate_velocity:
                    uice_grid = np.rot90(uice_grid, axes=(2, 3))
                    vice_grid = np.rot90(vice_grid, axes=(2, 3))
                else:
                    var_grid = np.rot90(var_grid, axes=(2, 3))

            # put it into the big grid
            if field_name in ['UVEL', 'VVEL'] and rotate_velocity:
                uvel_aste_grid[:, :, r * aste_sNy:(r + 1) * aste_sNy, c * aste_sNx:(c + 1) * aste_sNx] = uvel_grid
                vvel_aste_grid[:, :, r * aste_sNy:(r + 1) * aste_sNy, c * aste_sNx:(c + 1) * aste_sNx] = vvel_grid
            elif field_name in ['SIuice', 'SIvice'] and rotate_velocity:
                uice_aste_grid[:, :, r * aste_sNy:(r + 1) * aste_sNy, c * aste_sNx:(c + 1) * aste_sNx] = uice_grid
                vice_aste_grid[:, :, r * aste_sNy:(r + 1) * aste_sNy, c * aste_sNx:(c + 1) * aste_sNx] = vice_grid
            else:
                aste_grid[:, :, r * aste_sNy:(r + 1) * aste_sNy, c * aste_sNx:(c + 1) * aste_sNx] = var_grid

    if field_name in ['UVEL', 'VVEL'] and rotate_velocity:
        uvel_out, vvel_out = rotate_aste_grids_to_natural_grids(uvel_aste_grid, vvel_aste_grid, aste_AngleCS, aste_AngleSN)
        if field_name == 'UVEL':
            aste_grid = uvel_out
        if field_name == 'VVEL':
            aste_grid = vvel_out

    if field_name in ['SIuice', 'SIvice'] and rotate_velocity:
        uice_out, vice_out = rotate_aste_grids_to_natural_grids(uice_aste_grid, vice_aste_grid, aste_AngleCS, aste_AngleSN)
...
def read_aste_seaice_pickup_to_stiched_grid(aste_dir,pickup_file,ordered_aste_tiles,ordered_aste_tile_rotations):
    ...
    for vn in range(len(var_names)):
        compact_var_grid = compact_var_grids[vn]
        if var_names[vn].lower() in ['sitices']:
            aste_grid = np.zeros((seaice_temp_Nr, aste_sNy * len(ordered_aste_tiles), aste_sNx * len(ordered_aste_tiles[0])))
        else:
            aste_grid = np.zeros((1, aste_sNy * len(ordered_aste_tiles), aste_sNx * len(ordered_aste_tiles[0])))
...
##################################################################################################
# grid to faces functions

def read_aste_compact_to_faces(var_path,dims,ordered_aste_tiles, ordered_aste_tile_rotations,dtype='>f8'):
    var_grid = np.fromfile(var_path, dtype)
    print(np.size(var_grid))
    llc = 270
    sNx = sNy = 90
    var_grid_faces = {}
    points_counted = 0
    ...
    if dims==2:
        for i in [1, 3, 4, 5]:
            grid_subset = var_grid[points_counted:points_counted + face_tile_dimensions[i][0] * sNx * face_tile_dimensions[i][1] * sNy]
            var_grid_faces[i] = np.reshape(grid_subset,
                                           (face_tile_dimensions[i][0] * sNy, face_tile_dimensions[i][1] * sNx))
            points_counted += face_tile_dimensions[i][0] * sNx * face_tile_dimensions[i][1] * sNy
    if dims == 3:
        for i in [1,3,4,5]:
            grid_subset = var_grid[points_counted:points_counted + 50*face_tile_dimensions[i][0]*sNx*face_tile_dimensions[i][1]*sNy]
            var_grid_faces[i] = np.reshape(grid_subset, (50, face_tile_dimensions[i][0]*sNy,face_tile_dimensions[i][1]*sNx))
            points_counted += 50*face_tile_dimensions[i][0]*sNx*face_tile_dimensions[i][1]*sNy

    var_grid_faces_subsets = var_grid_faces#{}
    # var_grid_faces_subsets[1] = var_grid_faces[1][-90:, :270]
    # var_grid_faces_subsets[3] = var_grid_faces[3][-270:, :90]

    return(var_grid_faces_subsets)


##################################################################################################
# rotation functions

def rotate_aste_grids_to_natural_grids(uvel, vvel, aste_AngleCS, aste_AngleSN):
