...
def stack_files_to_nc(output_dir, output_file, subset_folder, subset,
    ...
    if subset == 'EtaN_day_snap':
        ...
        var_names = ['Uvel','Vvel']
    elif subset == 'BGC_daily_Chl':
        var_names = ['Chl01','Chl02','Chl03','Chl04','Chl05']
    elif subset == 'BGC_daily_consts':
        var_names = ['DIC','NO3','NO2','NH4','PO4','FeT','SiO2']
    elif subset == 'BGC_daily_cx':
        var_names = ['c01','c02','c03','c04','c05','c06','c07']
    elif subset == 'BGC_daily_DO':
        var_names = ['DOC','DON','DOP','DOFe']
    elif subset == 'BGC_daily_misc':
        var_names = ['PIC','Alk','O2']
    elif subset == 'BGC_daily_PO':
        var_names = ['POC','PON','POP','POFe','POSi']
    else:
    ...
    for iter_number in iteration_subset:
        ...
        if subset in ['BGC_daily_Chl','BGC_daily_consts','BGC_daily_cx','BGC_daily_DO','BGC_daily_misc','BGC_daily_PO','TS_AW_daily_snap']:
            grid = np.reshape(grid, (len(var_names), np.shape(XC)[0], np.shape(XC)[1]))
            output_array[:, counter, :, :] = grid
        elif subset in ['EtaN_day_snap','EXF_day_snap','SI_daily_snap','TS_surf_daily_snap']:
            grid = np.reshape(grid,(len(var_names),np.shape(XC)[0],np.shape(XC)[1]))
            output_array[:,counter,:,:] = grid
        else:
...
def stack_data_to_nc(config_dir, subset):
    ...
    if subset=='All':
        subsets = ['BGC_daily_consts','BGC_daily_cx','BGC_daily_DO','BGC_daily_misc','BGC_daily_PO','BGC_daily_Chl',
                   'EtaN_day_snap','SI_daily_snap','TS_surf_daily_snap','TS_AW_daily_snap']
    else:
        subsets = [subset]
