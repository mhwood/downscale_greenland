...
def read_L1_CE_Greenland_boundary_condition(config_dir, L1_model_name, boundary, field_name,
                                             Nr, n_rows_L1, n_cols_L1, n_timesteps, year):
    ...
    if boundary == 'east' or boundary == 'west':
        boundary_file = os.path.join(config_dir, 'L1_grid', L1_model_name, 'input', 'obcs',
                                     'L1_BC_' + boundary + '_' + field_name.upper() + '_' + str(year))
        boundary_grid = np.fromfile(boundary_file, '>f4')
        boundary_grid = boundary_grid.reshape((n_timesteps, Nr, n_rows_L1))
...
def create_2D_BC_plot(config_dir, L1_model_name, var_name, timestep,
    ...
    output_file = os.path.join(config_dir, 'L1_grid', L1_model_name, 'plots', 'init_files',
                               L1_model_name + '_BC_' + var_name + '_' + str(timestep) + '.png')
...
def plot_L1_CE_Greenland_BCs(config_dir, L1_model_name, print_level):
    ...
    for var_name in var_names:
        ...
        north_boundary = read_L1_CE_Greenland_boundary_condition(config_dir, L1_model_name, 'north', var_name,
                                                                  Nr, n_rows_L1, n_cols_L1, n_timesteps, year)
        north_boundary = north_boundary[timestep, :, :]

        south_boundary = read_L1_CE_Greenland_boundary_condition(config_dir, L1_model_name, 'south', var_name,
                                                                  Nr, n_rows_L1, n_cols_L1, n_timesteps, year)
        south_boundary = south_boundary[timestep, :, :]

        east_boundary = read_L1_CE_Greenland_boundary_condition(config_dir, L1_model_name, 'east', var_name,
                                                                 Nr, n_rows_L1, n_cols_L1, n_timesteps,year)
        east_boundary = east_boundary[timestep, :, :]

        west_boundary = read_L1_CE_Greenland_boundary_condition(config_dir, L1_model_name, 'west', var_name,
                                                                Nr, n_rows_L1, n_cols_L1, n_timesteps,year)
