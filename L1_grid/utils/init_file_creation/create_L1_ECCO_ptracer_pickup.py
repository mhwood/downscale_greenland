...
def create_L1_ECCO_ptracer_pickup_file(config_dir, model_name,
    ...
    pickup_file = 'pickup_ptracers.'+'{:010d}'.format(parent_model_pickup_iteration)
    if read_darwin:
    ...
    var_names, var_grids, global_metadata = ef.read_ecco_ptracer_pickup_to_stiched_grid(pickup_file_path, ordered_ecco_tiles, ordered_ecco_tile_rotations)
    ...
    output_file = os.path.join(output_dir, 'pickup_ptracers.' + '{:010d}'.format(4*parent_model_pickup_iteration))
    dtype = '>f8'
