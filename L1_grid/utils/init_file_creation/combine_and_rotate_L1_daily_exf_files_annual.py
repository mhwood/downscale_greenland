...
def get_dest_file_list(model_name, var_name, n_rows, n_cols, year):
    ...
    for month in range(1, 13):
        ...
        for day in range(1, nDays + 1):
            ...
            dest_file_shapes[dest_file] = (nTimesteps, n_rows, n_cols)
...
def read_stitched_grid_tile_angles(config_dir,model_name,sNx,sNy,ordered_nonblank_tiles,ordered_nonblank_rotations):
    ...
    grid_dir = os.path.join(config_dir, 'L1_grid', model_name, 'run_for_grid')
...
def stack_daily_exf_files_to_one(config_dir, model_name, var_name,
    ...
    for dest_file in dest_files:
        ...
        if var_name in ['UWIND','VWIND','USTRESS','VSTRESS']:
            ...
            u_var_grid = np.fromfile(os.path.join(config_dir, 'L1_grid',model_name, 'input', 'exf', 'U'+var_name[1:], u_dest_file),'>f4')
            v_var_grid = np.fromfile(os.path.join(config_dir, 'L1_grid',model_name, 'input', 'exf', 'V'+var_name[1:], v_dest_file),'>f4')
            ...
            var_grid = np.fromfile(os.path.join(config_dir, 'L1_grid', model_name, 'input', 'exf', var_name, dest_file), '>f4')
...
def combine_L1_daily_exf_files_annual(config_dir, model_name, var_name,
                                     start_year, final_year,
                                     print_level):

    XC, YC, AngleCS, AngleSN = read_grid_geometry(config_dir, model_name)
    ...
    for year in range(start_year,final_year+1):
        ...
        dest_files, dest_file_shapes, total_timesteps = get_dest_file_list(model_name, var_name, n_rows, n_cols, year)

        if var_name in ['UWIND','VWIND','USTRESS','VSTRESS']:
            if print_level >= 2:
                print('        - Choosing the grid orientationn')

            AngleCS_subset = np.copy(AngleCS)
            AngleSN_subset = np.copy(AngleSN)
        ...
        output_file = os.path.join(config_dir, 'L1_grid', model_name, 'input', 'exf', 'L1_exf_' + var_name+ '_' + str(year))
        if print_level >= 1:
